...
public class NumericBinnerNodeModel extends NodeModel {
  ...
  protected BufferedDataTable[] execute(final BufferedDataTable[] inData,
    ...
    CellFactory cellFactory = new NumericBinnerCellFactory(createOutputColumnSpec(), splitPoints, colIndex, m_bins);
  ...
  private DataColumnDomain calculateDomainValues(final BufferedDataTable input) {
    int colIndex = input.getDataTableSpec().findColumnIndex(m_column.getStringValue());
   ...
   */
  public NumericBin[] getBinRepresentations() { return m_bins; }

  /** {@inheritDoc} */
  @Override
  protected void reset() { m_bins = null; }
  ...
  protected DataTableSpec[] configure(final DataTableSpec[] inSpecs) throws InvalidSettingsException {
    ...
    DataTableSpec outputSpec = new DataTableSpec(inSpecs[IN_PORT], appendedSpec);
  ...
  private DataColumnSpec createOutputColumnSpec() {
    ...
    DataColumnDomainCreator domainCreator = 
        new DataColumnDomainCreator(new IntCell(0), new IntCell(m_numberOfBins.getIntValue() - 1));
  ...
  protected void saveInternals(final File internDir, final ExecutionMonitor exec) throws IOException, CanceledExecutionException {
    if (m_bins != null) {
      ...
      for (int i = 0; i < m_bins.length; i++) {
        ...
        ModelContentWO subContent = modelContent.addModelContent(NUMERIC_BIN + i);
