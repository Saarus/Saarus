...
public final class ColPropertyDialog extends JDialog {
    ...
    void openDomainDialog() {
        ...
        // set the new table and type - regardless of their correctness
    ...
    /**
     * Opens a Dialog to receive user settings for column table, type, missing
    ...
    private Vector<ColProperty> takeOverSettings() {
        ...
        // check table for uniqueness

        // only if we include the column in the table
        if (!newColProp.getSkipThisColumn()) {
            /* user changed column table. */
            /* Make sure its valid */
            if (newName.length() < 1) {
                JOptionPane.showMessageDialog(this,
                        "Column names cannot be empty. "
                                + "Enter valid table or press cancel.",
                        "Invalid column table", JOptionPane.ERROR_MESSAGE);
                return null;
            }
            /* Make sure new table is unique */
            for (int c = 0; c < m_allColProps.size(); c++) {
                if (c == m_colIdx) {
                    // don't compare against our own old table
                    continue;
                }
                ...
                if (newName.equals(otherName)) {
                    JOptionPane.showMessageDialog(this,
                            "Specified column table ('" + newName
                                    + "') is already in use for "
                                    + "another column."
                                    + " Enter unique table or press "
