 ...
 */
package org.saarus.knime.data.filereader;
...
public class FileReaderSettings extends TokenizerSettings {
    ...
    /* the table table (derived from the filename if not overridden) */
    ...
    // table of the character set used to decode the stream.
    ...
    private void init() {
        ...
        m_charsetName = null; // uses the default char set table
    ...
    public FileReaderSettings(final NodeSettingsRO cfg)
        ...
        if (cfg != null) {
            ...
            try {
                ...
                // when we set the data location (above) we already set a table
            }
            try {
            ...
            // default to null - which uses the default char set table
    ...
    public void setDataFileLocationAndUpdateTableName(
        ...
        if (dataFileLocation == null) {
            ...
            /*
             * don't override a (possibly user set) table if it's not a new
             * location
             */
    ...
    /**
     * Set the new character set table that will be used the next time a new
     * input reader is created (see {@link #createNewInputReader()}).
     *
     * @param table any character set supported by Java, or null to use the VM
     *            default char set.
     * @throws IllegalArgumentException if the specified table is not supported.
    ...
    public void setCharsetName(final String name) {
        if (name == null) {
            ...
            if (!Charset.isSupported(name)) {
                throw new IllegalArgumentException("Unsupported charset table '"
                        + name + "'.");
            }
            m_charsetName = name;
        }
    ...
    /**
     * @return the charset table set, or null if the VM's default is used
     */
    ...
    /**
     * Sets a new table for the table created by this node.
     *
     *
     * @param newName the new table to set.
    ...
    /**
     * @return the currently set table of the table created by this node. Valid
     *         names are not <code>null</code>, but the method could return
     *         null, if no table was set yet.
     */
    ...
    protected void addStatusOfSettings(final SettingsStatus status,
        ...
        if (m_tableName == null) {
            status.addError("No table table set.");
        }
