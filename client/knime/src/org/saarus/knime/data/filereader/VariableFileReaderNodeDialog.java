...
public class VariableFileReaderNodeDialog extends NodeDialogPane implements
    ...
    private JPanel createPreviewPanel() {
        ...
        hintBox.add(new JLabel("Click column header to change "
                + "column properties (* = table/type user settings)"));
        hintBox.add(Box.createGlue());
     ...
     * column. This will allow the user to enter new column table, type and
    ...
    protected void setNewUserSettingsForColumn(final int colIdx) {
        ...
        if (newColProps != null) {
            ...
            // user changed column type/table, we can clear that warning
            setAnalWarningText("");
            updatePreview();
        }
    ...
    private void loadSettingsFromInternal(final NodeSettingsRO settings,
        ...
        // set the URL, if available, or clear an invalid variable table
        try {
    ...
    /*
     * Reads the entered variable table from the edit field and stores the new
     ...
     * data file table text field or the force parameter is set true. It
     ...
     * if the table and type of the column is user set, and specially marked if
    ...
    private FileReaderNodeSettings createPreviewSettings(
        ...
        for (int c = 0; c < numCols; c++) {
            ...
            int idx = 2; // the index we append to the table to make it unique
            String colName = name;
     ...
     * the only thing left is the file table from the data file location text
     * field.
     */
     ...
     * are not set by the user. The flag indicating a user set table is the
