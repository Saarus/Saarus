...
public class FileReaderNodeSettings extends FileReaderSettings {
    ...
    private void addThisStatus(final SettingsStatus status) {
        ...
        for (int c = 0; c < propsToCheck; c++) {
            ...
            // check column spec, i.e. table, type and possible values
            if (cProp.getColumnSpec() == null) {
                status.addError("Column table and type not specified for column"
                        + c);
                continue;
            }
            // check the table
            ...
            if (cName == null) {
                status.addError("No column table specified for column " + c);
            }
            // and type
