...
public class FileImportPanel extends JPanel {
  ...
  private JTextField description, tableName ;
  ...
  private JPanel createInputBox(FileImportConfig config) {
    ...
    description = new JTextField(config.getDescription()) ;
    ...
    tableName = new JTextField(config.getTable()) ;
    ...
    autoDetect.addActionListener(new ActionListener() {
      public void actionPerformed(final ActionEvent e) {
        try {
          ...
          TableMetadata tableMetadata = hclient.descTable(tableName.getText(), false) ;
  ...
  FileImportConfig getFileImportConfig() {
