...
CREATE TABLE user (     
  ...
  LOCATION '/user/hive/yelpdb/warehouse/user';

INSERT OVERWRITE TABLE user 
  SELECT u.user_id, u.name, u.average_stars, u.review_count, v.funny, v.useful, v.cool
    FROM user_json 
    LATERAL VIEW json_tuple(
                   user_json.json, 'user_id', 'name', 'average_stars', 'review_count', 'votes') 
      u AS user_id, name, average_stars, review_count, votes  
      LATERAL VIEW json_tuple(u.votes, 'funny', 'useful', 'cool') v AS funny, useful, cool;
...
CREATE TABLE checkin(
     ...
     checkin_info         STRING)   
  ROW FORMAT DELIMITED FIELDS TERMINATED BY "," ESCAPED BY '\\' STORED AS TEXTFILE
  LOCATION '/user/hive/yelpdb/warehouse/checkin';
...
CREATE TABLE review(     
     ...
     vote_cool      INT)
  ROW FORMAT DELIMITED FIELDS TERMINATED BY "," ESCAPED BY '\\' STORED AS TEXTFILE
  LOCATION '/user/hive/yelpdb/warehouse/review';
