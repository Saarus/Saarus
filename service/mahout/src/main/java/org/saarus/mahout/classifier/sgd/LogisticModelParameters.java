...
public class LogisticModelParameters implements Writable {
  ...
  private RecordFactoryImpl csv;
  private OnlineLogisticRegression lr;

  /**
   * Returns a RecordFactoryImpl compatible with this logistic model.  The reason that this is tied
   ...
   * suffice.
   *
   * @return The RecordFactoryImpl.
   */
  public RecordFactoryImpl getCsvRecordFactory() {
    if (csv == null) {
      csv = new RecordFactoryImpl(getTargetVariable(), getTypeMap())
   ...
   * @param typeList      The list of types in the format preferred by RecordFactoryImpl.
   */
  public void setTypeMap(List<PredictorType> predictorTypes) {
    Preconditions.checkArgument(!predictorTypes.isEmpty(), "Must have at least one type specifier");
    typeMap = Maps.newHashMap();
    for (PredictorType pt : predictorTypes) {
      typeMap.put(pt.getName(), pt.getType());
    }
  }
  ...
  public boolean useBias() { return useBias; }

  public String getTargetVariable() { return targetVariable; }

  public Map<String, String> getTypeMap() { return typeMap; }
  public void setTypeMap(Map<String, String> map) { this.typeMap = map; }

  public int getNumFeatures() { return numFeatures; }

  public int getMaxTargetCategories() { return maxTargetCategories; }
